...
def calculate_multiclass_f1_score(preds, labels):
    ...
    #preds = torch.argmax(torch_preds).detach().numpy()
    #labels = labels.numpy()
    #print(labels)
    #print(torch_labels)
...
def calculate_multiclass_accuracy(preds, labels):
    ...
    #print(preds)
    #print(preds.shape)
    preds = torch.argmax(torch_preds)
    acc = float(torch.sum(torch.eq(torch_labels, preds)).data) / torch_labels.size(0)
    return acc
...
class EarlyStopping(object):
    ...
    def __init__(self, patience=7, verbose=True, wandb=None, name=""):
        """
        Args:
