...
from atari_seed_wrapper import patch_atari_seed
...
def parse_args():
    ...
    parser.add_argument("--total-timesteps", type=int, default=100000, #10000000 -> Modified, max 100k steps
...
if __name__ == "__main__":
    ...
    game_name = args.env_id.split("/")[-1].split("-")[0]

    def reboot_env(env_seed=None) -> gym.vector.SyncVectorEnv:
        global ale_py
        if args.patch_env_seed:
            if env_seed is not None:
                ale_py = patch_atari_seed(ale_py, env_seed, game_name)
            else:
                raise ValueError("Please provide a valid `env_seed`.")
    ...
    envs = reboot_env(args.env_seeds[0])
    ...
    if args.patch_env_seed:
        ...
        episodic_length = defaultdict(list)
        episodic_test_steps = defaultdict(list)

        print(f"Running agent validation with seeds {', '.join(test_seeds)}")

        for i, test_seed in enumerate(test_seeds):
            print(f"Test {i+1}/{num_tests}, seed={test_seed}")

            envs = reboot_env(test_seed)
            ...
            # TODO limit episodes/num_steps or run until done = True?
            for update in range(1, args.test_episodes + 1):
                for step in range(0, args.num_steps):
                    test_step += 1 * args.num_envs
                    obs[step] = next_obs
                    dones[step] = next_done
                    ...
                    for item in info:
                        if "episode" in item.keys():
                            print(
                                f"test_step={test_step}, episodic_return={item['episode']['r']}"
                            )
                            episodic_returns[f"seed={test_seed}"].append(
                                item["episode"]["r"]
                            )
                            episodic_length[f"seed={test_seed}"].append(
                                item["episode"]["l"]
                            )
                            episodic_test_steps[f"seed={test_seed}"].append(test_step)
                            break
