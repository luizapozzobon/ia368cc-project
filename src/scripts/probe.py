...
class probe_ds(torch.utils.data.Dataset):
...
class FullySupervisedLinearProbe(nn.Module):
    def __init__(self, encoder, num_classes=255):
        ...
        self.probe = LinearProbe(input_dim=self.encoder.feature_size,
...
class ProbeTrainer():
    ...
    def create_probes(self, sample_label):
        ...
        self.early_stoppers = {
            k: EarlyStopping(patience=self.patience, verbose=False, name=k + "_probe")
    ...
    def do_one_epoch(self, episodes, label_dicts):
        ...
        train_ds = probe_ds(episodes, label_dicts)
        data_generator = DataLoader(train_ds, batch_size=self.batch_size, shuffle=True)
        for step, (x, labels_batch) in enumerate(data_generator):
            for labeld in labels_batch:
                for k, label_value in labeld.items():
                    if self.early_stoppers[k].early_stop:
                        continue
                    optim = self.optimizers[k]
                    optim.zero_grad()

                    label = torch.tensor(label_value).long().to(self.device)
    ...
    def do_test_epoch(self, episodes, label_dicts):
        ...
        test_ds = probe_ds(episodes, label_dicts)
        data_generator = DataLoader(test_ds, batch_size=self.batch_size, shuffle=False)
        # collect all predictions first
        #data_generator = self.generate_batch(episodes, label_dicts)
        for step, (x, labels_batch) in enumerate(data_generator):
            for labeld in labels_batch:
                for k, label_value in labeld.items():
                    label = torch.tensor(label_value).long().cpu()
